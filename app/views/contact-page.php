...
<body class="g-sidenav-show  bg-gray-100">
  ...
  <main class="main-content position-relative border-radius-lg ">
    ...
    <!-- contact Page Management Content -->
          ...
          <div class="card">
            <div class="card-header pb-0">
              <div class="d-flex justify-content-between align-items-center">
                <h5 class="m-0">Manajemen Halaman Contact</h5>
                <div>
                  <button class="btn btn-sm btn-success mb-0" id="btn-save-contact">
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="text" name="contact_header_title" class="form-control" placeholder="contact">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="mb-3">
                          <label class="form-label">Subjudul Header</label>
                          <input type="text" name="contact_header_subtitle" class="form-control" placeholder="Hubungi kami untuk informasi lebih lanjut">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="file" name="contact_header_image" class="form-control" accept=".jpg,.jpeg,.png,.gif,.webp">
                          <div class="form-text text-xs">Format: JPG, PNG, GIF, WebP (Maks. 5MB)</div>
                          <div class="mt-2" id="contact-header-image-preview">
                ...
                <div class="card m-4">
                  <div class="card-body">
                    <div class="alert alert-warning text-white mb-0">
                      ...
                      <strong>Informasi:</strong> Informasi kontak detail (alamat, telepon, email, form kontak) dikelola melalui <strong>Pengaturan Website</strong>.
                      <a href="<?= BASE_URL ?>/settings" class="alert-link">Kelola pengaturan di sini</a>.
                    </div>
                  </div>
                </div>
              </form>
    ...
    <!-- End contact Page Management Content -->
  </main>
  ...
  <script>
    ...
    function showExistingImagePreviews(contents) {
      ...
      imageFields.forEach(field => {
        ...
        if (contents[field] && contents[field].value) {
          $(`#${previewId}`).html(`
            <img src="${BASE_URL}/uploads/contact/${contents[field].value}" alt="Current ${field}" class="img-thumbnail" style="max-height: 120px;">
    ...
    // Read contact Page Data
    function readContactData() {
      $.ajax({
        url: BASE_URL + '/contact-page/read',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          if (res.success && res.data) {
            // Header Section
            $('#form-contact input[name="contact_header_title"]').val(res.data.header?.title || '');
            $('#form-contact input[name="contact_header_subtitle"]').val(res.data.header?.subtitle || '');
            ...
            showAlert('Gagal memuat data konten halaman contact', 'error');
          }
        },
        error: function(xhr, status, error) {
          let errorMessage = 'Terjadi kesalahan saat membaca data konten halaman contact.';
    ...
    $('#btn-save-contact').on('click', function() {
      ...
      $.ajax({
        url: BASE_URL + '/contact-page/update',
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(res) {
          if (res.success) {
            ...
            showAlert(res.message || 'Gagal menyimpan konten halaman contact.', 'error');
          }
        },
        error: function(xhr) {
          let errorMessage = 'Terjadi kesalahan saat menyimpan konten halaman contact.';
          if (xhr.responseJSON && xhr.responseJSON.message) {
            errorMessage = xhr.responseJSON.message;
          }
          showAlert(errorMessage, 'error');
