...
<body class="g-sidenav-show  bg-gray-100">
  ...
  <main class="main-content position-relative border-radius-lg ">
          ...
          <div class="card">
            <div class="card-header pb-0">
              <div class="d-flex justify-content-between align-items-center">
                <h5 class="m-0">Manajemen Halaman Berita</h5>
                <div>
                  <button class="btn btn-sm btn-success mb-0" id="btn-save-news-page">
            ...
            <div class="card-body p-0">
              <form id="form-news-page" enctype="multipart/form-data">
                <!-- Header Section -->
                <div class="card m-4">
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="text" name="news_header_title" class="form-control" placeholder="Berita & Artikel">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="mb-3">
                          <label class="form-label">Subjudul Header</label>
                          <input type="text" name="news_header_subtitle" class="form-control" placeholder="Informasi terbaru dari LAB IVSS">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="file" name="news_header_image" class="form-control" accept=".jpg,.jpeg,.png,.gif,.webp">
                          <div class="form-text text-xs">Format: JPG, PNG, GIF, WebP (Maks. 5MB)</div>
                          <div class="mt-2" id="news-header-image-preview">
                ...
                <div class="card m-4">
                  <div class="card-body">
                    <div class="alert alert-warning text-white mb-0">
                      ...
                      <strong>Informasi:</strong> Konten berita (artikel) dikelola melalui <strong>Manajemen Berita</strong>.
                      <a href="<?= BASE_URL ?>/berita" class="alert-link">Kelola berita di sini</a>.
                    </div>
                  </div>
                </div>
              </form>
    ...
    <!-- End News Page Management Content -->
  </main>
  ...
  <script>
    ...
    function showExistingImagePreviews(contents) {
      ...
      imageFields.forEach(field => {
        ...
        if (contents[field] && contents[field].value) {
          $(`#${previewId}`).html(`
            <img src="${BASE_URL}/uploads/news/${contents[field].value}" alt="Current ${field}" class="img-thumbnail" style="max-height: 120px;">
    ...
    // Read News Page Data
    function readNewsPageData() {
      $.ajax({
        url: BASE_URL + '/berita-page/read',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          ...
          if (res.success && res.data) {
            // Header Section
            $('#form-news-page input[name="news_header_title"]').val(res.data.header?.title || '');
            $('#form-news-page input[name="news_header_subtitle"]').val(res.data.header?.subtitle || '');
            ...
            showAlert('Gagal memuat data konten halaman berita', 'error');
          }
        },
        error: function(xhr, status, error) {
          let errorMessage = 'Terjadi kesalahan saat membaca data konten halaman berita.';
    ...
    // Save News Page Contents
    $('#btn-save-news-page').on('click', function() {
      const btn = $(this);
      const originalText = btn.html();

      btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Menyimpan...');

      const formData = new FormData($('#form-news-page')[0]);

      // Kirim semua data sekaligus
      $.ajax({
        url: BASE_URL + '/berita-page/update',
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(res) {
          if (res.success) {
            ...
            showAlert(res.message || 'Gagal menyimpan konten halaman berita.', 'error');
          }
        },
        error: function(xhr) {
          let errorMessage = 'Terjadi kesalahan saat menyimpan konten halaman berita.';
          if (xhr.responseJSON && xhr.responseJSON.message) {
            errorMessage = xhr.responseJSON.message;
          }
          showAlert(errorMessage, 'error');
    ...
    $(document).ready(function() {
      ...
      $('#form-news-page').on('submit', function(e) {
        e.preventDefault();
        $('#btn-save-news-page').click();
      });
    });
  </script>
