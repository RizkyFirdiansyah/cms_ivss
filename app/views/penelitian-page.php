...
<body class="g-sidenav-show  bg-gray-100">
  ...
  <main class="main-content position-relative border-radius-lg ">
    ...
    <!-- Research Page Management Content -->
          ...
          <div class="card">
            <div class="card-header pb-0">
              <div class="d-flex justify-content-between align-items-center">
                <h5 class="m-0">Manajemen Halaman Penelitian</h5>
                <div>
                  <button class="btn btn-sm btn-success mb-0" id="btn-save-research-page">
            ...
            <div class="card-body p-0">
              <form id="form-research-page" enctype="multipart/form-data">
                <!-- Header Section -->
                <div class="card m-4">
                    ...
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="text" name="research_header_title" class="form-control" placeholder="Penelitian">
                        </div>
                      </div>
                      <div class="col-md-6">
                        <div class="mb-3">
                          <label class="form-label">Subjudul Header</label>
                          <input type="text" name="research_header_subtitle" class="form-control" placeholder="Proyek dan kegiatan penelitian LAB IVSS">
                        </div>
                      </div>
                    </div>
                    <div class="row">
                      <div class="col-md-6">
                        <div class="mb-3">
                          ...
                          <input type="file" name="research_header_image" class="form-control" accept=".jpg,.jpeg,.png,.gif,.webp">
                          <div class="form-text text-xs">Format: JPG, PNG, GIF, WebP (Maks. 5MB)</div>
                          <div class="mt-2" id="research-header-image-preview">
                ...
                <div class="card m-4">
                  <div class="card-body">
                    <div class="alert alert-warning text-white mb-0">
                      ...
                      <strong>Informasi:</strong> Konten penelitian dikelola melalui <strong>Manajemen Penelitian</strong>.
                      <a href="<?= BASE_URL ?>/penelitian" class="alert-link">Kelola penelitian di sini</a>.
                    </div>
                  </div>
                </div>
              </form>
    ...
    <!-- End Research Page Management Content -->
  </main>
  ...
  <script>
    ...
    function showExistingImagePreviews(contents) {
      ...
      imageFields.forEach(field => {
        ...
        if (contents[field] && contents[field].value) {
          $(`#${previewId}`).html(`
            <img src="${BASE_URL}/uploads/research/${contents[field].value}" alt="Current ${field}" class="img-thumbnail" style="max-height: 120px;">
    ...
    // Read Research Page Data
    function readResearchPageData() {
      $.ajax({
        url: BASE_URL + '/penelitian-page/read',
        method: 'GET',
        dataType: 'json',
        success: function(res) {
          if (res.success && res.data) {
            // Header Section
            $('#form-research-page input[name="research_header_title"]').val(res.data.header?.title || '');
            $('#form-research-page input[name="research_header_subtitle"]').val(res.data.header?.subtitle || '');
            ...
            showAlert('Gagal memuat data konten halaman penelitian', 'error');
          }
        },
        error: function(xhr, status, error) {
          let errorMessage = 'Terjadi kesalahan saat membaca data konten halaman penelitian.';
    ...
    // Save Research Page Contents
    $('#btn-save-research-page').on('click', function() {
      const btn = $(this);
      const originalText = btn.html();

      btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin me-1"></i> Menyimpan...');

      const formData = new FormData($('#form-research-page')[0]);

      // Kirim semua data sekaligus
      $.ajax({
        url: BASE_URL + '/penelitian-page/update',
        method: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        success: function(res) {
          if (res.success) {
            ...
            setTimeout(() => {
              readResearchPageData();
            }, 1000);
          } else {
            showAlert(res.message || 'Gagal menyimpan konten halaman penelitian.', 'error');
          }
        },
        error: function(xhr) {
          let errorMessage = 'Terjadi kesalahan saat menyimpan konten halaman penelitian.';
          if (xhr.responseJSON && xhr.responseJSON.message) {
            errorMessage = xhr.responseJSON.message;
          }
          showAlert(errorMessage, 'error');
    ...
    $(document).ready(function() {
      ...
      $('#form-research-page').on('submit', function(e) {
        e.preventDefault();
        $('#btn-save-research-page').click();
      });
    });
  </script>
