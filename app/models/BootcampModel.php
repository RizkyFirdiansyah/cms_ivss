...
class BootcampModel
{
    ...
    // Di dalam class BootcampModel (tambahkan method berikut):

    // Get bootcamp by ID untuk API
    public function getBootcampByIdForApi($id)
    {
        try {
            $bootcamp = $this->getById($id);
            ...
            $formattedBootcamp = [
                ...
                'link' => $bootcamp['link'],
                'created_by_user_id' => $bootcamp['created_by_user_id'],
                'author_name' => $bootcamp['author_name'],
                'created_at' => $bootcamp['created_at'],
                'categories' => $bootcamp['categories']
            ];

            return $formattedBootcamp;
        } catch (PDOException $e) {
            error_log("DB Error (getBootcampByIdForApi): " . $e->getMessage());
            return null;
        }
    }

    // Count bootcamps by year (jika diperlukan)
    public function countBootcampsByYear($year)
    {
        try {
            ...
            error_log("DB Error (countBootcampsByYear): " . $e->getMessage());
            return 0;
        }
    }
}
