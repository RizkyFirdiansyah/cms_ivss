...
class HomeModel
{
  ...
  public function getHomeContent($key)
  {
    try {
      ...
      $query = "SELECT content_value, content_type 
                FROM page_contents 
  ...
  // Save home content
  public function saveHomeContent($key, $value, $type, $userId)
  {
    try {
      $pageId = $this->getPageId('home');
      if (!$pageId) {
      ...
      $stmt->bindValue(':content_key', $key, PDO::PARAM_STR);
      $stmt->bindValue(':content_type', $type, PDO::PARAM_STR);
      $stmt->bindValue(':content_value', $value, PDO::PARAM_STR);

      return $stmt->execute();
    } catch (PDOException $e) {
      error_log("DB Error (saveHomeContent): " . $e->getMessage());
      return false;
    }
  }
